...
C = 16
x, y = clean_data("data/L_poker_" + player + "_C=16_LUCB_exploitability_log", LUCB_num_experiments, num_iterations, step_size, omit_range, multiplier, interpolation = False)
x_mccfr, y_mccfr = clean_data("data/L_poker_" + player + "_MCCFR_OS_exploitability_log", MCCFR_num_experiments, num_iterations, step_size, omit_range, multiplier, interpolation = False)
x_opf, y_opf = clean_data("data/Lpoker_eps_constant=0.100000_" + player + "onpath_flipping_exploitability_log", On_path_flipping_num_experiments, num_iterations, step_size, omit_range, multiplier, interpolation = False)

plt.plot(x, y, '-', linewidth=1, color='blue', label='LUCB for player ' + player + ', C = ' + str(C) + ', ' + str(LUCB_num_experiments) + ' experiments')
plt.plot(x_mccfr, y_mccfr, '-.', linewidth=1, color='red', label='MCCFR Outcome Sampling for player ' + player + ', ' + str(MCCFR_num_experiments) + ' experiments')
plt.plot(x_opf, y_opf, ':', linewidth=1, color='green', label='On-Path Flipping for player ' + player + ', ' + str(On_path_flipping_num_experiments) + ' experiments')
...
plt.legend()
plt.savefig('Leduc_x_all_3_new.pdf')
plt.show()
