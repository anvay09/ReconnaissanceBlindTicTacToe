...
double sample_terminal_history(InformationSet& I_1, InformationSet& I_2, InformationSet prev_opponent_I, int prev_opponent_action,
                               ...
                               std::vector<std::vector<double>>& action_LCB, std::vector<std::vector<double>>& R, 
        ...
        if (success && !true_cards.is_win(winner) && !true_cards.is_over()) {
            ...
            if (I.player == 'x') {   
                if (true_cards.player_to_move == 'x'){
                    return sample_terminal_history(new_I, I_2, prev_opponent_I, prev_opponent_action, true_cards, player_policy, opponent_policy, current_history, trajectory, br_player, action_UCB, action_LCB, R, infoset_reach_count, terminal_reach_count, game);
                }
                else {
                    return sample_terminal_history(new_I, I_2, I, action, true_cards, player_policy, opponent_policy, current_history, trajectory, br_player, action_UCB, action_LCB, R, infoset_reach_count, terminal_reach_count, game);
                }
            } else {
                if (true_cards.player_to_move == 'o'){
                    return sample_terminal_history(I_1, new_I, prev_opponent_I, prev_opponent_action, true_cards, player_policy, opponent_policy, current_history, trajectory, br_player, action_UCB, action_LCB, R, infoset_reach_count, terminal_reach_count, game);
                }
                else {
                    return sample_terminal_history(I_1, new_I, I, action, true_cards, player_policy, opponent_policy, current_history, trajectory, br_player, action_UCB, action_LCB, R, infoset_reach_count, terminal_reach_count, game);
                }
            }
        } else {
...
void updateBounds(std::vector<std::vector<double>>& R, std::vector<int>& infoset_reach_count, std::vector<std::vector<int>>& terminal_reach_count, 
                  std::vector<std::vector<double>>& action_UCB, std::vector<std::vector<double>>& action_LCB, std::vector<std::pair<std::string, 
...
void algorithm(double eps, double delta, double gamma, int S, int H, char br_player, PolicyVec& player_policy, PolicyVec& opponent_policy, std::vector<std::string>& player_information_sets, int T, int log_freq, char game, double br_value, int experiment_number){
    ...
    for (int t = 1; t <= T; t++){
        ...
        // sample game
        double reward = sample_terminal_history(I_1, I_2, I_2, 0, true_cards, player_policy, opponent_policy, start_history, trajectory, br_player, action_UCB, action_LCB, R, infoset_reach_count, terminal_reach_count, game);
        // update bounds
        updateBounds(R, infoset_reach_count, terminal_reach_count, action_UCB, action_LCB, trajectory, br_player, eps, delta, gamma, S, H, t, game);
    }

    std::cout << "Saving exploitability log" << std::endl;
    std::string file_name = "data/gapE/" + std::string(1, br_player) + "_" + std::string(1, game) + "_BPIUCRL_exploitability_log_" + std::to_string(experiment_number) + ".txt";
...
int main(int argc, char* argv[]) {
    ...
    while (experiment_number <= num_experiments){
        ...
        if (player == 'x'){
            ...
            algorithm(eps, delta, gamma, S, H, player, player_br_policy, policy_obj_o, P1_information_sets, num_iterations, log_freq, game, expected_utility, experiment_number);
        }
        else if (player == 'o'){
            PolicyVec uniform_policy_obj_o('o', P2_information_sets, game, false);
            PolicyVec player_br_policy = uniform_policy_obj_o;
            algorithm(eps, delta, gamma, S, H, player, player_br_policy, policy_obj_x, P2_information_sets, num_iterations, log_freq, game, expected_utility, experiment_number);
        }
        experiment_number += 1;
    }
}
