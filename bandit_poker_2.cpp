...
double solve_for_max_q_KLUCB(int Na, double emp_mean, double beta){
    double upper = 1.0;
    double lower = emp_mean;
    double q = (upper + lower) / 2.0;

    while (upper - lower > 1e-6){
        if ((double) Na * KL_divergence(emp_mean, q) < beta) {
            lower = q;
        }
        else {
            upper = q;
...
void update_max_reward_policy_given_history(InformationSet& I, PokerTable& true_cards, InformationSet& opponent_I, History& game, PolicyVec& max_reward_policy, 
                                            std::vector<int>& infoset_reach_count, std::vector<std::vector<std::vector<int>>>& empirical_action_reward, 
                                            std::vector<std::vector<int>>& action_terminal_reach_count, std::vector<double>& infoset_values, char br_player, int traversal_index) {
    ...
    if (curr_player == br_player){
        if (I.move_flag) {
            ...
            update_max_reward_policy_given_history(new_I, new_board, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, br_player, traversal_index + 1);
        }
        else {
            InformationSet new_I = I;
            new_I.simulate_sense(action, true_cards);
            update_max_reward_policy_given_history(new_I, true_cards, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, br_player, traversal_index + 1);
        }
        ...
        if (opponent_I.move_flag){
            ...
            update_max_reward_policy_given_history(I, true_cards, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, br_player, traversal_index + 1);
        }
        else {
...
double build_max_UCB_policy(PolicyVec& policy_obj, InformationSet& I, std::vector<int>& infoset_reach_count, std::vector<int>& I_tickmark, 
    ...
    for (int a : legal_actions){
        ...
        for (std::string I_prime_hash : cohort){
            ...
            //     double max_util = I_prime.game == 'L'? LEDUC_MAX_UTILITY : KUHN_MAX_UTILITY;
            //     action_ucb_values[a] += infoset_reach_count[I_prime.get_index()] * max_util;
            // }
            // else {
            action_ucb_values[a] += infoset_reach_count[I_prime.get_index()] * cohort_ucb_values[I_prime_hash];
            // }
        }
        ...
        if (u != 0){
            ...
            action_ucb_values[a] += sqrt(C * log(infoset_time_step[I.get_index()]) / u); // might need global t, squared
...
void update_max_UCB_policy_given_history(InformationSet& I, PokerTable& true_cards, InformationSet& opponent_I, History& game, PolicyVec& max_UCB_policy, std::vector<int>& infoset_reach_count, 
                                         std::vector<std::vector<std::vector<int>>>& empirical_action_reward, std::vector<std::vector<int>>& action_terminal_reach_count, 
    ...
    if (curr_player == br_player){
        if (I.move_flag) {
            ...
            new_I.update_move(action);
            new_board.update_move(action);
            update_max_UCB_policy_given_history(new_I, new_board, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }
        else {
            InformationSet new_I = I;
            new_I.simulate_sense(action, true_cards);
            update_max_UCB_policy_given_history(new_I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }   
        ...
        if (opponent_I.move_flag){
            opponent_I.update_move(action);
            true_cards.update_move(action);
            update_max_UCB_policy_given_history(I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }
        else {
            opponent_I.simulate_sense(action, true_cards);
            update_max_UCB_policy_given_history(I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
...
double build_max_KLUCB_policy(PolicyVec& policy_obj, InformationSet& I, std::vector<int>& infoset_reach_count, std::vector<int>& I_tickmark, 
                            std::vector<std::vector<std::vector<int>>>& empirical_action_reward, std::vector<std::vector<int>>& action_terminal_reach_count, 
                            std::vector<int>& infoset_time_step, int C, std::vector<int>& success_metrics, std::vector<double>& infoset_ucb_values, std::vector<std::vector<int>>& success_metrics_pi_hat, std::vector<std::vector<int>>& action_explore_count){
    std::vector<int> legal_actions;
    I.get_actions(legal_actions);
    std::vector<double> action_klucb_values(6, 0.0);
    double max_klucb = I.game == 'L'? LEDUC_MIN_UTILITY : KUHN_MIN_UTILITY;

    for (int a : legal_actions){
        std::unordered_set<std::string> cohort;
        std::unordered_map<std::string, double> cohort_klucb_values;
        get_cohort(I, a, cohort);
        int u = 0;
        int terminal_reach_count = action_terminal_reach_count[I.get_index()][a]; // number of times action led to terminal state
        ...
        for (std::string I_prime_hash : cohort){
            ...
            cohort_klucb_values[I_prime_hash] = build_max_KLUCB_policy(policy_obj, I_prime, infoset_reach_count, I_tickmark, empirical_action_reward, action_terminal_reach_count, infoset_time_step, C, success_metrics_prime, infoset_ucb_values, success_metrics_pi_hat, action_explore_count);


            if (I_tickmark[I_prime.get_index()] == 0){
                u += infoset_reach_count[I_prime.get_index()];
                action_klucb_values[a] += infoset_reach_count[I_prime.get_index()];
            }
            else {
                int denom = success_metrics_prime[0] + success_metrics_prime[1] + success_metrics_prime[2];
                if (denom != 0){
                    u += denom;
                    action_klucb_values[a] += (success_metrics_prime[0] - success_metrics_prime[2]);
        ...
        if (terminal_reach_count != 0){
            u += terminal_reach_count;
            action_klucb_values[a] += (empirical_action_reward[I.get_index()][a][0] - empirical_action_reward[I.get_index()][a][2]);
        ...
        if (u != 0){
            action_klucb_values[a] /= u;
            double beta = std::log(infoset_time_step[I.get_index()]) + C * std::log(std::log(infoset_time_step[I.get_index()]));
            action_klucb_values[a] = solve_for_max_q_KLUCB(u, action_klucb_values[a], beta);
            infoset_time_step[I.get_index()] += 1;
        }
        else {
            action_klucb_values[a] = I.game == 'L'? LEDUC_MAX_UTILITY : KUHN_MAX_UTILITY;
        }
    }

    for (int a : legal_actions){
        if (action_klucb_values[a] > max_klucb){
            max_klucb = action_klucb_values[a];
        }
    }
    ...
    if (I_tickmark[I.get_index()] == 0){
        ...
        std::vector<int> candidate_actions;
        for (int a : legal_actions){
            if (fabs(action_klucb_values[a] - max_klucb) < 1e-6){
                candidate_actions.push_back(a);
            }
        }
        ...
        infoset_ucb_values[I.get_index()] = max_klucb;
        success_metrics_pi_hat[I.get_index()] = success_metrics;
        return max_klucb;
    }
}
...
double build_max_KLUCB_policy_parallel(PolicyVec& policy_obj, InformationSet& I, std::vector<int>& infoset_reach_count, std::vector<int>& I_tickmark, 
                                     std::vector<std::vector<std::vector<int>>>& empirical_action_reward, std::vector<std::vector<int>>& action_terminal_reach_count, 
                                     std::vector<int>& infoset_time_step, int C, std::vector<int>& success_metrics, std::vector<double>& infoset_ucb_values, std::vector<std::vector<int>>& success_metrics_pi_hat, std::vector<std::vector<int>>& action_explore_count){
    std::vector<int> legal_actions;
    I.get_actions(legal_actions);
...
void update_max_KLUCB_policy_given_history(InformationSet& I, PokerTable& true_cards, InformationSet& opponent_I, History& game, PolicyVec& max_UCB_policy, std::vector<int>& infoset_reach_count, 
                                         std::vector<std::vector<std::vector<int>>>& empirical_action_reward, std::vector<std::vector<int>>& action_terminal_reach_count, 
                                         std::vector<double>& infoset_ucb_values, char br_player, int traversal_index, std::vector<int>& infoset_time_step, 
                                         int C, std::vector<int>& I_tickmark, std::vector<std::vector<int>>& success_metrics_pi_hat, std::vector<std::vector<int>>& action_explore_count) {
    if (traversal_index == game.history.size()){
    ...
    if (curr_player == br_player){
        if (I.move_flag) {
            ...
            new_I.update_move(action);
            new_board.update_move(action);
            update_max_KLUCB_policy_given_history(new_I, new_board, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }
        else {
            InformationSet new_I = I;
            new_I.simulate_sense(action, true_cards);
            update_max_KLUCB_policy_given_history(new_I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }   

        std::vector<int> legal_actions;
        I.get_actions(legal_actions);
        std::vector<double> action_klucb_values(6, 0.0);
        double max_klucb = true_cards.game == 'L'? LEDUC_MIN_UTILITY : KUHN_MIN_UTILITY;
        std::vector<int> success_metrics{0, 0, 0};
        ...
        for (int a : legal_actions){
            ...
            for (std::string I_prime_hash : cohort){
                ...
                std::vector<int> success_metrics_prime = success_metrics_pi_hat[I_prime.get_index()];
                cohort_klucb_values[I_prime_hash] = infoset_ucb_values[I_prime.get_index()];
            ...
            if (u != 0){
                ...
                action_klucb_values[a] = true_cards.game == 'L'? LEDUC_MAX_UTILITY : KUHN_MAX_UTILITY;
            }
        }
        ...
        if (opponent_I.move_flag){
            opponent_I.update_move(action);
            true_cards.update_move(action);
            update_max_KLUCB_policy_given_history(I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
        }
        else {
            opponent_I.simulate_sense(action, true_cards);
            update_max_KLUCB_policy_given_history(I, true_cards, opponent_I, game, max_UCB_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, traversal_index + 1, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
...
void calc_br_IS_KLLUCB(PolicyVec& opponent_policy, char br_player, std::vector<std::string>& player_information_sets, int log_frequency, int m, PolicyVec& player_br, int experiment_number, char game, int iterations, int C, std::string& exp_name) {    
    std::vector<std::vector<int>> I_a_tickmark(player_information_sets.size(), std::vector<int>(6, 0));
    std::vector<int> I_tickmark(player_information_sets.size(), 0);
    ...
    for (int card_index = 0; card_index < player_cards.size(); card_index++){
        ...
        InformationSet root = br_player == 'x' ? InformationSet('x', true, hash_1, game) : InformationSet('o', false, hash_2, game);
        std::vector<int> success_metrics{0, 0, 0};
        double max_UCB = build_max_KLUCB_policy(player_max_ucb_policy, root, infoset_reach_count, I_tickmark, empirical_action_reward, action_terminal_reach_count, infoset_time_step, C, success_metrics, infoset_ucb_values, success_metrics_pi_hat, action_explore_count);
    }
    ...
    for (; t <= iterations; t += 1) {
        ...
        if (max_UCB_flag) {
            ...
            if (br_player == 'x') {
                update_max_KLUCB_policy_given_history(I_1, true_cards, I_2, start_history, player_max_ucb_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, 3, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
            }
            else {
                update_max_KLUCB_policy_given_history(I_2, true_cards, I_1, start_history, player_max_ucb_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_ucb_values, br_player, 3, infoset_time_step, C, I_tickmark, success_metrics_pi_hat, action_explore_count);
            }
    ...
    std::cout << "Saving exploitability log" << std::endl;
    std::string file_name = "data/bandit/" + exp_name + "_" + std::string(1, game) + "_poker_" + std::string(1, br_player) + "_C=" + std::to_string(C) + "_IS-KLLUCB_exploitability_log_" + std::to_string(experiment_number) + ".txt";
...
int main(int argc, char* argv[]) {
    ...
    while (experiment_num < experiments + start_index) {
        if (player == 'x') {
            ...
            if (algorithm == "KLLUCB") {
                calc_br_IS_KLLUCB(policy_obj_o, 'x', P1_information_sets, log_frequency, 1, uniform_x, experiment_num, game, iterations, C, exp_name);
            }
            else if (algorithm == "LUCB") {
            ...
            if (algorithm == "KLLUCB") {
                calc_br_IS_KLLUCB(policy_obj_x, 'o', P2_information_sets, log_frequency, 1, uniform_o, experiment_num, game, iterations, C, exp_name);
            }
            else if (algorithm == "LUCB") {
