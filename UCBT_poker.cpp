...
// this code is a version of sequence UCB (regret minimisation) which uses the sequence with the minimum n_hat count to decide the UCB of the policy
...
{  
    ...
    std::string file_name = "data/sequence/" + exp_name + "_" + std::string(1, game) + "_poker_" + std::string(1, br_player) + "_C=" + std::to_string(C) + "_C_r=" + std::to_string(C_r) + "_seq-UCB_exploitability_log_" + std::to_string(experiment_number) + ".txt";
    ...
    file_name = "data/sequence/" + exp_name + "_" + std::string(1, game) + "_poker_" + std::string(1, br_player) + "_C=" + std::to_string(C) + "_C_r=" + std::to_string(C_r) + "_seq-UCB_reward_log_" + std::to_string(experiment_number) + ".txt";
...
int main(int argc, char* argv[]) {
    ...
    while (experiment_num < experiments + start_index) {
        ...
        if (player == 'x') {
            ...
            calc_br_sequence_UCB(policy_obj_o, 'x', P1_information_sets, log_frequency, random_x, experiment_num, game, iterations, C, C_r, exp_name, sequence_hash_to_index_map, terminal_sequences, cohorts, delta);
        }
        else {
            PolicyVec random_o('o', P2_information_sets, game, false);
            calc_br_sequence_UCB(policy_obj_x, 'o', P2_information_sets, log_frequency, random_o, experiment_num, game, iterations, C, C_r, exp_name, sequence_hash_to_index_map, terminal_sequences, cohorts, delta);
        }
