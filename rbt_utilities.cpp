    ...
    if (I.move_flag) {
        for (int a = 0; a < actions.size(); a++) {
            ...
            if (depth_3_history_list.size() > 0) {
                ...
                new_I.update_move(actions[a], I.player);
                new_I.reset_zeros();
                Q_values[actions[a]] += compute_worst_response(new_I, br_player, depth_3_true_board_list, depth_3_history_list, depth_3_reach_probability_list, depth_3_opponent_I_list, br, policy_obj);
            }
        }
    }
    else {
        for (int a = 0; a < actions.size(); a++) {
            ...
            for (int t = 0; t < infoset_set.size(); t++) {
                ...
                if (infoset_to_history[new_I.hash].size() > 0) {
                    Q_values[actions[a]] += compute_worst_response(new_I, br_player, infoset_to_true_board[new_I.hash], infoset_to_history[new_I.hash], infoset_to_reach_probability[new_I.hash], infoset_to_opponent_I[new_I.hash], br, policy_obj);
                }
            }
        }
    }

    return get_min_Q_value_and_update_policy(Q_values, actions, br, I);
}
...
double compute_worst_response_parallel(InformationSet& I, char br_player, std::vector<TicTacToeBoard>& true_board_list, std::vector<History>& history_list, 
    ...
    if (I.move_flag) {
        ...
        for (int t = 0; t < infoset_set.size(); t++) {
            ...
            if (infoset_to_history[new_I.hash].size() > 0) {
                depth_4_Q_values[a_val][b_val] += compute_worst_response(new_I, br_player, infoset_to_true_board[new_I.hash], infoset_to_history[new_I.hash], infoset_to_reach_probability[new_I.hash], infoset_to_opponent_I[new_I.hash], br, policy_obj);
            }
        }
        ...
        for (int a = 0; a < actions.size(); a++) {
            if (action_to_history_list[actions[a]].size() > 0) {
                ...
                Q_values[actions[a]] = get_min_Q_value_and_update_policy(depth_4_Q_values[actions[a]], depth_4_actions[actions[a]], br, new_I);
            }
        }
    }
    else {
        ...
        for (int t = 0; t < depth_2_infoset_set.size(); t++) {
            ...
            if (depth_2_infoset_to_history[new_I.hash].size() > 0) {
                depth_2_Q_values[parent_hash][b_val] += compute_worst_response(new_I, br_player, depth_2_infoset_to_true_board[new_I.hash], depth_2_infoset_to_history[new_I.hash], depth_2_infoset_to_reach_probability[new_I.hash], depth_2_infoset_to_opponent_I[new_I.hash], br, policy_obj);
            }
        }
        ...
        for (int t = 0; t < infoset_set.size(); t++) {
            ...
            if (infoset_to_history[new_I.hash].size() > 0) {
                int a_val = infoset_to_action_taken[new_I.hash];
                Q_values[a_val] += get_min_Q_value_and_update_policy(depth_2_Q_values[new_I.hash], depth_2_actions[new_I.hash], br, new_I);
            }
        }
...
double compute_worst_response_wrapper(PolicyVec& policy_obj, PolicyVec& br, char br_player) {
    ...
    if (br_player == 'x') {
        ...
        expected_utility = compute_worst_response_parallel(I_1, br_player, true_board_list, history_list, reach_probability_list, opponent_I_list, br, policy_obj);
    } 
    else {
        std::vector<int> actions;
        I_1.get_actions_given_policy(actions, policy_obj);
        ...
        expected_utility = - compute_worst_response_parallel(I_2, br_player, true_board_list, history_list, reach_probability_list, opponent_I_list, br, policy_obj);
    }
