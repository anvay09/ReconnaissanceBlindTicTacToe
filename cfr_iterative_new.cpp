...
double get_prob_h_given_policy_prob(InformationSet& I_1, InformationSet& I_2, TicTacToeBoard& true_board, char player, int next_action, 
...
double get_prob_h_given_policy_wrapper_prob(InformationSet& I_1, InformationSet& I_2, 
                                       ...
                                       PolicyVec& policy_obj_o, double probability,
...
void get_probability_of_reaching_all_h_prob(InformationSet& I, PolicyVec& policy_obj_x, PolicyVec& policy_obj_o, std::vector<std::vector<int>>& starting_histories, 
    ...
    for (std::vector<int> h : starting_histories) {
        ...
        if (!(I.board == "000000000")) {
            ...
            InformationSet I_2('o', false, hash_2);
            TicTacToeBoard true_board = TicTacToeBoard(board);
            double probability_reaching_h = get_prob_h_given_policy_wrapper_prob(I_1, I_2, true_board, 'x', h[0], policy_obj_x, policy_obj_o, 1.0, h_object, I, initial_player);
...
//avg
void calc_average_terms(char player, std::vector<std::string>& information_sets, PolicyVec& policy_obj, std::vector<double>& prob_reaching_list, std::vector<std::vector<double>>& avg_policy_numerator, std::vector<double>& avg_policy_denominator){
