...
double sample_terminal_history(InformationSet &I_1, InformationSet &I_2, TicTacToeBoard &true_board, PolicyVec &policy_obj_x, PolicyVec &policy_obj_o, History &current_history, char player, double probability, double &reward, char update_player, double eps, PolicyVec &player_uniform_policy)
{
    ...
    if (I.move_flag)
    {
        ...
        if (success && !true_board.is_win(winner) && !true_board.is_over())
        {
            ...
            if (player == 'x')
            {
                return sample_terminal_history(new_I, I_2, true_board, policy_obj_x, policy_obj_o, current_history, 'o', probability, reward, update_player, eps, player_uniform_policy);
            }
            else
            {
                return sample_terminal_history(I_1, new_I, true_board, policy_obj_x, policy_obj_o, current_history, 'x', probability, reward, update_player, eps, player_uniform_policy);
            }
        }
        else
        {
    ...
    else
    {
        ...
        if (player == 'x')
        {
            return sample_terminal_history(new_I, I_2, true_board, policy_obj_x, policy_obj_o, current_history, 'x', probability, reward, update_player, eps, player_uniform_policy);
        }
        else
        {
            return sample_terminal_history(I_1, new_I, true_board, policy_obj_x, policy_obj_o, current_history, 'o', probability, reward, update_player, eps, player_uniform_policy);
        }
    }
}

double sample_terminal_history_wrapper(PolicyVec &policy_obj_x, PolicyVec &policy_obj_o, History &current_history, double &reward, char update_player, double eps, PolicyVec &player_uniform_policy)
{
    ...
    return sample_terminal_history(I_1, I_2, true_board, policy_obj_x, policy_obj_o, current_history, 'x', 1.0, reward, update_player, eps, player_uniform_policy);
}
...
void mccfr_outcome_sampling_best_response(PolicyVec &opponent_policy, PolicyVec &player_br_policy, char br_player, std::vector<std::string> &player_information_sets, long int T, double eps, int experiment_number, long int log_size, double exact_br_value, PolicyVec &player_uniform_policy, std::string base_path)
{
    ...
    for (int t = 0; t < T; t++)
    {
        ...
        if (br_player == 'x')
        {
            q_z = sample_terminal_history_wrapper(player_br_policy, opponent_policy, start_history, reward, br_player, eps, player_uniform_policy);
        }
        else
        {
            q_z = sample_terminal_history_wrapper(opponent_policy, player_br_policy, start_history, reward, br_player, eps, player_uniform_policy);
        }
    ...
    std::string file_name = base_path + "/" + std::string(1, br_player) + "_mccfr_greedy_" + std::to_string(experiment_number) + ".txt";
    std::string file_name_average = base_path + "/" + std::string(1, br_player) + "_average_mccfr_greedy_" + std::to_string(experiment_number) + ".txt";
...
int main(int argc, char *argv[])
{
    ...
    while (experiment_number <= num_experiments)
    {
        if (player == 'x')
        {
            ...
            mccfr_outcome_sampling_best_response(policy_obj_o, player_br_policy, 'x', P1_information_sets, num_iterations, eps, experiment_number, log_size, expected_utility, uniform_policy_obj_x, base_path);
        }
        else if (player == 'o')
        {
            PolicyVec player_br_policy = uniform_policy_obj_o;
            mccfr_outcome_sampling_best_response(policy_obj_x, player_br_policy, 'o', P2_information_sets, num_iterations, eps, experiment_number, log_size, expected_utility, uniform_policy_obj_o, base_path);
        }
        experiment_number += 1;
    }
}