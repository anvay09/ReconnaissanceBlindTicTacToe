...
void upfront_flipping_best_response(PolicyVec& opponent_policy, PolicyVec& player_br_policy, PolicyVec& player_uniform_policy, char br_player, std::vector<std::string>& player_information_sets, long int T, long int step_size, double exact_br_value, int experiment_number, long int log_size, double eps, int decay_flag) {
    ...
    if (decay_flag) {
        file_name = "data/eps_decay/step_size=" + std::to_string(step_size) + "/" + std::string(1, br_player) + "upfront_flipping_exploitability_log_" + std::to_string(experiment_number) + ".txt";
        file_name_average = "data/eps_decay/step_size=" + std::to_string(step_size) + "/" + std::string(1, br_player) + "average_upfront_flipping_exploitability_log_" + std::to_string(experiment_number) + ".txt";

    }
    else {
        file_name = "data/eps_constant/eps=" + std::to_string(eps) + "/" + std::string(1, br_player) + "upfront_flipping_exploitability_log_" + std::to_string(experiment_number) + ".txt";
        file_name_average = "data/eps_constant/eps=" + std::to_string(eps) + "/" + std::string(1, br_player) + "average_upfront_flipping_exploitability_log_" + std::to_string(experiment_number) + ".txt";
    }
