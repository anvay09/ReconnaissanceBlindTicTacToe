...
def parallel_play(I_1, I_2, true_board, player, p1_policy_obj, p2_policy_obj, current_player_run, cfr_round, base_path):
    Total_histories = 0
    I_1_vars = []
    I_2_vars = []
    true_board_vars = []
...
def parse_commandline_args():
    ...
    return arguments.CurrentPlayer, arguments.PolicyFileX, arguments.PolicyFileO, int(arguments.Round), int(
        arguments.ReachableISFlag), int(arguments.FilterValidHistoriesFlag), arguments.BasePath
...
if __name__ == "__main__":
    ...
    if cfr_player == 'o':
        reachable_IS_file_player = './{}/P2_information_sets_10.txt'.format(base_path)
        valid_histories_file_player = './{}/valid_histories/p2_valid_histories_for_reachable_I_round_{}.json'.format(base_path, cfr_round)
    else:
        reachable_IS_file_player = './{}/P1_information_sets_10.txt'.format(base_path)
    ...
    expected_utility = get_expected_utility_parallel(I_1, I_2, true_board, player, policy_obj_x, policy_obj_o, 1,
