...
def parallel_cpp(file_path_1, file_path_2, round):
    os.system("./evaluate_policy" + " " + file_path_1 + " " + file_path_2)
    
    outfile = "expected_utility.txt"
...
if __name__ == "__main__":
    ...
    for round in x_range:
        ...
        expected_utility_list.append(parallel_cpp(p1_file, p2_file, round))

   

    X = [i for i in range(start_round, num)]
    Y = [float(exp_util) for exp_util in expected_utility_list]
    ...
    with open('{}/expected_util/{}.json'.format(arguments.BasePath, arguments.OutputFileName), 'w') as f:
        json.dump({"0": expected_utility_list}, f)
