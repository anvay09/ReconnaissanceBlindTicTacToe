...
AI_player = None
P1_file_name = "/Users/anvay/Desktop/ReconnaissanceBlindTicTacToe/data/P1_linear_nash_v2_normalised.txt"
P2_file_name = "/Users/anvay/Desktop/ReconnaissanceBlindTicTacToe/data/P2_linear_nash_v2_normalised.txt"
P1_stats_file = "/Users/anvay/Desktop/ReconnaissanceBlindTicTacToe/P1_linear_nash_v2_normalised_stats.json"
P2_stats_file = "/Users/anvay/Desktop/ReconnaissanceBlindTicTacToe/P2_linear_nash_v2_normalised_stats.json"

use_policy = True

# load policies
print('Loading policies...')
P1_policy = PolicyVec('x', P1_file_name)
P2_policy = PolicyVec('o', P2_file_name)
print('Policies loaded')
P1_sense_buffer = 0
P2_sense_buffer = choices([i for i in range(13)], P2_policy.policy_dict[P2_hash_to_int_map[""]])[0]
stats_flag = 0
...
p1_boardview = InformationSet('x', True, "")
p2_boardview = InformationSet('o', False, "")
...
sense_index_to_sense_coordinates_map = {9: (100, 100), 10: (200, 100), 11: (100, 200), 12: (200, 200)}
...
def draw_board(s):
    ...
    if winning_line and game_over and winner is not None:
        ...
        for move in moves:
            ...
            if turn:
                if p2_boardview.board[ind] != '0' and p2_boardview.board[ind] != '-':
                    draw_shape(move[0], move[1], s, move[2])
            else:
                if p1_boardview.board[ind] != '0' and p1_boardview.board[ind] != '-':
                    draw_shape(move[0], move[1], s, move[2])
...
def make_move(move):
    ...
    if move[2]:
        board[ind] = 'o'
        p2_boardview.update_move(ind, 'o')
    else:
        board[ind] = 'x'
        p1_boardview.update_move(ind, 'x')
...
def update_boardview(sense_coordinates):
    ...
    if turn:
        p2_boardview.simulate_sense(sense_action, board)

    else:
        p1_boardview.simulate_sense(sense_action, board)
...
def move_action(event, last_click_time, square, surface):
    ...
    if (move_coordinates[0], move_coordinates[1], turn) and (
        ...
        if turn:
            if p2_boardview.board[ind] == '0' or p2_boardview.board[ind] == '-':
                if event.type == pygame.MOUSEBUTTONDOWN:
                    current_time = pygame.time.get_ticks()
                    if current_time - last_click_time > click_delay:
                        game_over = True
            ...
            if p1_boardview.board[ind] == '0' or p1_boardview.board[ind] == '-':
                if event.type == pygame.MOUSEBUTTONDOWN:
                    current_time = pygame.time.get_ticks()
                    if current_time - last_click_time > click_delay:
                        game_over = True
    ...
    if choose_player_screen:
        ...
        if use_policy and not turn and AI_player == 1:
            if sensing:
                ...
                if p1_boardview.get_index() == -1:
                    print('Error: Invalid information set, line 562: ', p1_boardview.board)
                    exit(1)

                # choose a move uniformly at random from P1_policy[P1_information_set]
                prob_dict = P1_policy.policy_dict[p1_boardview.get_index()]
                chosen_play = choices([i for i in range(13)], prob_dict)[0]
                move_coordinates = board_index_to_coordinates_map[int(chosen_play)]

                p1_boardview_copy = p1_boardview.copy()
                p1_boardview_copy.update_move(int(chosen_play), 'x')

                if p1_boardview_copy.get_index() == -1:
                    P1_sense_buffer = 0
                else:
                    prob_dict = P1_policy.policy_dict[p1_boardview_copy.get_index()]
                    P1_sense_buffer = choices([i for i in range(13)], prob_dict)[0]
            ...
            if sensing:
                ...
                if p2_boardview.get_index() == -1:
                    print('Error: Invalid information set, line 601: ', p2_boardview.board, p2_boardview.get_hash())
                    exit(1)

                # choose a move uniformly at random from P2_policy[P2_information_set]
                prob_dict = P2_policy.policy_dict[p2_boardview.get_index()]
                chosen_play = choices([i for i in range(13)], prob_dict)[0]
                move_coordinates = board_index_to_coordinates_map[int(chosen_play)]

                p2_boardview_copy = p2_boardview.copy()
                p2_boardview_copy.update_move(int(chosen_play), 'o')

                if p2_boardview_copy.get_index() == -1:
                    P2_sense_buffer = 0
                else:
                    prob_dict = P2_policy.policy_dict[p2_boardview_copy.get_index()]
                    P2_sense_buffer = choices([i for i in range(13)], prob_dict)[0]
