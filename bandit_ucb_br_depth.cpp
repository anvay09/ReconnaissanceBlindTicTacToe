...
double sample_terminal_history(InformationSet& I_1, InformationSet& I_2, TicTacToeBoard& true_board, std::vector<std::vector<double>>& infoset_ucb_values, std::vector<std::vector<double>>& infoset_empirical_reward, std::vector<std::vector<long int>>& infoset_pull_count, long int timestep, long int C, PolicyVec& opponent_policy, History& current_history, char player, char br_player, long int k) {
    ...
    if (player == br_player){ // choose action with max UCB value
        ...
        for (int a : legal_actions){
            if (pull_counts[a] > 0){
                ...
                action_ucbs[a] = emp_rewards[a] + sqrt(exploration_bonus*log(timestep) / pull_counts[a]);
            }
        }
    ...
    if (I.move_flag) {
        ...
        if (success && !true_board.is_win(winner) && !true_board.is_over()) {
            ...
            if (player == 'x') {
                return sample_terminal_history(new_I, I_2, true_board, infoset_ucb_values, infoset_empirical_reward, infoset_pull_count, timestep, C, opponent_policy, current_history, 'o', br_player, k);
            } else {
                return sample_terminal_history(I_1, new_I, true_board, infoset_ucb_values, infoset_empirical_reward, infoset_pull_count, timestep, C, opponent_policy, current_history, 'x', br_player, k);
            }
        } else {
            TerminalHistory H_T = TerminalHistory(current_history.history);
            H_T.set_reward();
        ...
        if (player == 'x') {
            return sample_terminal_history(new_I, I_2, true_board, infoset_ucb_values, infoset_empirical_reward, infoset_pull_count, timestep, C, opponent_policy, current_history, 'x', br_player, k);
        } else {
            return sample_terminal_history(I_1, new_I, true_board, infoset_ucb_values, infoset_empirical_reward, infoset_pull_count, timestep, C, opponent_policy, current_history, 'o', br_player, k);
        }
    }
}


double sample_terminal_history_wrapper(std::vector<std::vector<double>>& infoset_ucb_values, std::vector<std::vector<double>>& infoset_empirical_reward, std::vector<std::vector<long int>>& infoset_pull_count, long int timestep, long int C, PolicyVec& opponent_policy, History& current_history, char br_player, long int k) {
    std::string board = "000000000";
    TicTacToeBoard true_board = TicTacToeBoard(board);
    std::string hash_1 = "";
    std::string hash_2 = "";
    InformationSet I_1 = InformationSet('x', true, hash_1);
    InformationSet I_2 = InformationSet('o', false, hash_2);
    return sample_terminal_history(I_1, I_2, true_board, infoset_ucb_values, infoset_empirical_reward, infoset_pull_count, timestep, C, opponent_policy, current_history, 'x', br_player, k);
}
