...
void update_simple_average_strategies_recursive(InformationSet& I, char player, std::vector<TicTacToeBoard>& true_board_list, std::vector<History>& history_list, 
                                         double reach_probability_sigma_t, double reach_probability_br, std::vector<InformationSet>& opponent_I_list, 
                                         PolicyVec& br, PolicyVec& sigma_t, PolicyVec& sigma_t_next, int t) {
    std::vector<int> all_actions;
    I.get_actions(all_actions);
    ...
    if (I.move_flag) {
        for (int a = 0; a < actions.size(); a++) {
            ...
            if (depth_3_history_list.size() > 0) {
                ...
                update_simple_average_strategies_recursive(new_I, player, depth_3_true_board_list, depth_3_history_list, depth_1_reach_probability_sigma_t, depth_1_reach_probability_br, depth_3_opponent_I_list, br, sigma_t, sigma_t_next, t);
            }
        }
    }
    else {
        for (int a = 0; a < actions.size(); a++) {
            ...
            for (int j = 0; j < infoset_set.size(); j++) {
                ...
                if (infoset_to_history[new_I.hash].size() > 0) {
                    update_simple_average_strategies_recursive(new_I, player, infoset_to_true_board[new_I.hash], infoset_to_history[new_I.hash], depth_1_reach_probability_sigma_t, depth_1_reach_probability_br, infoset_to_opponent_I[new_I.hash], br, sigma_t, sigma_t_next, t);
                }
            }
        }
    }
...
void update_simple_average_strategies_recursive_parallel(InformationSet& I, char player, std::vector<TicTacToeBoard>& true_board_list, std::vector<History>& history_list, 
                                                  double reach_probability_sigma_t, double reach_probability_br, std::vector<InformationSet>& opponent_I_list, 
                                                  PolicyVec& br, PolicyVec& sigma_t, PolicyVec& sigma_t_next, int t){
    std::vector<int> all_actions;
    I.get_actions(all_actions);
    ...
    if (I.move_flag) {
        ...
        for (int a = 0; a < actions.size(); a++) {
            if (action_to_history_list[actions[a]].size() > 0) {
                ...
                new_I.update_move(actions[a], I.player);
                new_I.reset_zeros();
                update_simple_average_strategies_recursive(new_I, player, action_to_true_board_list[actions[a]], action_to_history_list[actions[a]], action_to_reach_probability_sigma_t[actions[a]], action_to_reach_probability_br[actions[a]], action_to_opponent_I_list[actions[a]], br, sigma_t, sigma_t_next, t);
            }
        }
    }
    else {
        ...
        for (int j = 0; j < infoset_set.size(); j++) {
            ...
            if (infoset_to_history[new_I.hash].size() > 0) {
                update_simple_average_strategies_recursive(new_I, player, infoset_to_true_board[new_I.hash], infoset_to_history[new_I.hash], infoset_to_reach_sigma_t[new_I.hash], infoset_to_reach_br[new_I.hash], infoset_to_opponent_I[new_I.hash], br, sigma_t, sigma_t_next, t);
            }
        }
    }
...
void update_simple_average_strategies_recursive_wrapper(PolicyVec& br, PolicyVec& sigma_t, PolicyVec& sigma_t_next, char player, int t) {
    std::string board = "000000000";
    TicTacToeBoard true_board = TicTacToeBoard(board);
    std::string hash_1 = "";
    std::string hash_2 = "";
    ...
    if (player == 'x') {
        ...
        update_simple_average_strategies_recursive_parallel(I_1, player, true_board_list, history_list, 1.0, 1.0, opponent_I_list, br, sigma_t, sigma_t_next, t);
    } 
    else {
        std::vector<int> actions;
        I_1.get_actions(actions);
        ...
        update_simple_average_strategies_recursive_parallel(I_2, player, true_board_list, history_list, 1.0, 1.0, opponent_I_list, br, sigma_t, sigma_t_next, t);
    }
...
void AFP(PolicyVec& x_bar_t, PolicyVec& y_bar_t, int T, std::vector<std::string>& P1_information_sets, std::vector<std::string>& P2_information_sets, PolicyVec& start_x, PolicyVec& start_o) {
    ...
    for (int t = 1; t <= T; t++) {
        ...
        update_simple_average_strategies_recursive_wrapper(br_x, x_bar_t, x_bar_t_intermediate, 'x', t);
        update_simple_average_strategies_recursive_wrapper(br_o, y_bar_t, y_bar_t_intermediate, 'o', t);
        ...
        update_simple_average_strategies_recursive_wrapper(br_x, x_bar_t, x_bar_t_next, 'x', t);
        update_simple_average_strategies_recursive_wrapper(br_o, y_bar_t, y_bar_t_next, 'o', t);
