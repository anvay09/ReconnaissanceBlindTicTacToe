...
void calc_br(PolicyVec& opponent_policy, char br_player, std::vector<std::string>& player_information_sets, int log_frequency, int m, PolicyVec& player_br, int experiment_number, std::string& exp_name) {
    ...
    std::string file_name = "data/" + exp_name + "_" + std::string(1, br_player) + "_pullm_exploitability_log_" + std::to_string(experiment_number) + ".txt";
...
int main(int argc, char* argv[]) {
    ...
    while (experiment_num <= experiments) {
        if (player == 'x') {
            ...
            calc_br(policy_obj_o, 'x', P1_information_sets, log_frequency, m, uniform_x, experiment_num, exp_name);
        }
        else {
            PolicyVec uniform_o('o', P2_information_sets);
            calc_br(policy_obj_x, 'o', P2_information_sets, log_frequency, m, uniform_o, experiment_num, exp_name);
        }
