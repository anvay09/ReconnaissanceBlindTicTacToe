...
// g++-13 -O3 rbt_classes.cpp get_games.cpp -o get_games
...
int main() {
    ...
    char player = 'x';
    std::string file_path_1 = "data/P1_linear_nash_lightweight.json";
    std::string file_path_2 = "data/P2_linear_nash_lightweight.json";
    Policy policy_obj_x('x', file_path_1);
    Policy policy_obj_o('o', file_path_2);

    std::vector<int> init_h = {};
    TerminalHistory start_history = TerminalHistory(init_h);
    ...
    // write terminal histories to file

    std::ofstream file;
    file.open("games.txt");
    for (auto& terminal_history : terminal_histories) {
        TerminalHistory H_T = std::get<0>(terminal_history);
        double probability = std::get<1>(terminal_history);
        double reward = std::get<2>(terminal_history);
        file << "History: ";
        for (int action : H_T.history) {
            file << action << " ";
        }
        file << "Probability: " << probability << " Reward: " << reward << std::endl;
    }
