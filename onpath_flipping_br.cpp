...
void onpath_flipping_best_response(PolicyVec& opponent_policy, PolicyVec& player_br_policy, PolicyVec& player_uniform_policy, char br_player, std::vector<std::string>& player_information_sets, long int T, long int step_size, double exact_br_value, int experiment_number, long int log_size) {
    ...
    while (continue_exp == 'y') {
        ...
        while (experiment_number <= num_experiments){
            if (player == 'x'){
                ...
                onpath_flipping_best_response(policy_obj_o, player_br_policy, uniform_policy_obj_x, 'x', P1_information_sets,  num_iterations, step_size, expected_utility, experiment_number, log_size);
            }
            else if (player == 'o'){
                PolicyVec player_br_policy = policy_obj_o;
                onpath_flipping_best_response(policy_obj_x, player_br_policy, uniform_policy_obj_o, 'o', P2_information_sets, num_iterations, step_size, expected_utility, experiment_number, log_size);
            }
            experiment_number += 1;
        }
