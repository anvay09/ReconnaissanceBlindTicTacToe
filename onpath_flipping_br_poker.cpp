    ...
    while (continue_exp == 'y') {
        ...
        while (experiment_number <= num_experiments){
            if (player == 'x'){
                ...
                onpath_flipping_best_response(policy_obj_o, player_br_policy, uniform_policy_obj_x, 'x', P1_information_sets,  num_iterations, step_size, expected_utility, experiment_number, log_size, eps, decay_flag, game, exp_name);
            }
            else if (player == 'o'){
                PolicyVec player_br_policy = policy_obj_o;
                onpath_flipping_best_response(policy_obj_x, player_br_policy, uniform_policy_obj_o, 'o', P2_information_sets, num_iterations, step_size, expected_utility, experiment_number, log_size, eps, decay_flag, game, exp_name);
            }
            experiment_number += 1;
        }
