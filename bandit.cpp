...
void update_max_reward_policy_given_history(InformationSet& I, TicTacToeBoard& true_board, InformationSet& opponent_I, History& game, PolicyVec& max_reward_policy, 
                                            std::vector<int>& infoset_reach_count, std::vector<std::vector<std::vector<int>>>& empirical_action_reward, 
                                            std::vector<std::vector<int>>& action_terminal_reach_count, std::vector<double>& infoset_values, char curr_player, char br_player, int traversal_index, PolicyVec& average_policy) {
    ...
    if (curr_player == br_player){
        if (I.move_flag) {
            ...
            update_max_reward_policy_given_history(new_I, new_board, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, toggle_player(curr_player), br_player, traversal_index + 1, average_policy);
        }
        else {
            InformationSet new_I = I;
            new_I.simulate_sense(action, true_board);
            I.reset_zeros();
            update_max_reward_policy_given_history(new_I, true_board, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, curr_player, br_player, traversal_index + 1, average_policy);
        }
        ...
        if (infoset_reach_count[I.get_index()] == 0){
            ...
            int action = candidate_actions[std::rand() % candidate_actions.size()];
            std::vector<double>& prob_dist = max_reward_policy.policy_dict[I.get_index()];
            std::vector<double>& average_prob_dist = average_policy.policy_dict[I.get_index()];
            double t = (double) infoset_reach_count[I.get_index()];

            for (int a : legal_actions){
                if (a == action){
                    prob_dist[a] = 1.0;
                    average_prob_dist[a] = (average_prob_dist[a] * (t - 1.0) + t) / (2*t - 1.0);
                }
                else{
                    prob_dist[a] = 0.0;
                    average_prob_dist[a] = (average_prob_dist[a] * (t - 1.0)) / (2*t - 1.0);
                }
            }
        ...
        if (opponent_I.move_flag){
            ...
            update_max_reward_policy_given_history(I, true_board, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, toggle_player(curr_player), br_player, traversal_index + 1, average_policy);
        }
        else {
            opponent_I.simulate_sense(action, true_board);
            update_max_reward_policy_given_history(I, true_board, opponent_I, game, max_reward_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, curr_player, br_player, traversal_index + 1, average_policy);
        }
    }
}
...
void calc_br(PolicyVec& opponent_policy, char br_player, std::vector<std::string>& player_information_sets, int log_frequency, int m, PolicyVec& player_br, int experiment_number) {
    ...
    PolicyVec player_average_br(br_player, player_information_sets);
    ...
    for (int j = 0; j <= iterations; j++) {
        if (j % 1000 == 0 && j != 0) { 
            ...
            if (br_player == 'x') {
                ...
                expected_utility = get_expected_utility_wrapper(player_average_br, opponent_policy);
                ...
                expected_utility = get_expected_utility_wrapper(opponent_policy, player_average_br);
                exploitability_log.push_back(std::make_pair(sample_count, exact_br_value - expected_utility));
            }
        ...
        for (int i = 0; i < samples; i++){
            ...
            InformationSet I_1 = InformationSet('x', true, hash_1);
            InformationSet I_2 = InformationSet('o', false, hash_2);
            update_max_reward_policy_given_history(I_1, true_board, I_2, start_history, player_br, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, 'x', br_player, 0, player_average_br);
            ...
            I_1 = InformationSet('x', true, hash_1);
            I_2 = InformationSet('o', false, hash_2);
            update_max_reward_policy_given_history(I_1, true_board, I_2, start_history, player_max_ucb_policy, infoset_reach_count, empirical_action_reward, action_terminal_reach_count, infoset_values, 'x', br_player, 0, player_average_br);
